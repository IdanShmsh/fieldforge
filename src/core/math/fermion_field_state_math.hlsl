...
namespace FermionFieldStateMath
{
    ...
    half norm_sqrd(FermionFieldState fermion_state)
    {
    ...
    void scl(FermionFieldState fermion_state, half2 scalar, out FermionFieldState result)
    {
    ...
    void rscl(FermionFieldState fermion_state, half real_scalar, out FermionFieldState result)
    {
    ...
    void scl_sum(FermionFieldState fermion_state1, FermionFieldState fermion_state2, half2 weight1, half2 weight2, out FermionFieldState result)
    {
    ...
    void rscl_sum(FermionFieldState fermion_state1, FermionFieldState fermion_state2, half weight1, half weight2, out FermionFieldState result)
    {
    ...
    void lerp_states(FermionFieldState fermion_state1, FermionFieldState fermion_state2, half weight, out FermionFieldState result)
    {
    ...
    half norm(FermionFieldState f1)
    {
        return sqrt(norm_sqrd(f1));
    }

    // Perform a phase rotation on a fermion state
    void phase_rot(FermionFieldState fermion_state, half phase_angle, out FermionFieldState result)
    {
    ...
    half2 inner_product(FermionFieldState fermion_state1, FermionFieldState fermion_state2)
    {
    ...
    void add_norm(FermionFieldState fermion_state, half norm_addition, out FermionFieldState result)
    {
        half state_norm = norm(fermion_state);
        if (state_norm == 0)
        {
            FermionFieldStateOps::empty(result);
            result[0] = half2(norm_addition, 0);
            return;
        }
        scl(fermion_state, half2((state_norm + norm_addition) / state_norm, 0), result);
    }

    // Clamp the norm of a fermion state to a specified range (without changing its direction and phase)
    void clamp_norm(FermionFieldState state, half clamp_min, half clamp_max, out FermionFieldState result)
    {
        half stete_norm = norm(state);
        if (stete_norm == 0)
        {
    ...
    void set_norm(FermionFieldState state, half target_norm, out FermionFieldState result)
    {
        result = state;
        half stete_norm = norm_sqrd(state);
        if (stete_norm == 0)
        {
    ...
    {
        half initial_norm = norm(fermion_state1);
    ...
    void harmonically_limit_norm(FermionFieldState fermion_state, half norm_limit, out FermionFieldState result)
    {
