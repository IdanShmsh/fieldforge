...
namespace FieldDenoising
{
    ...
    // algorithm with specified spatial and range standard deviations in a specified fermion lattice buffer
    // * Side Effects:
    // • Reads directly from the simulation's lattice buffers
    // • Writes directly to the simulation's lattice buffers
    void bilateral_denoise_fermion_state(float3 position, uint field_index, float spatial_std, float range_std, FermionLatticeBuffer fermion_lattice_buffer)
    {
        ...
        FermionFieldState center_state = fermion_lattice_buffer[center_index];
    ...
    // and range standard deviations in a specified fermion lattice buffer
    // * Side Effects:
    // • Reads directly from the simulation's lattice buffers
    // • Writes directly to the simulation's lattice buffers
    ...
    // • Reads directly from the simulation's lattice buffers
    // • Writes directly to the simulation's lattice buffers
    void bilateral_denoise_gauge_state(float3 position, uint symmetry_index, float spatial_std, float range_std, GaugeLatticeBuffer gauge_lattice_buffer)
    {
        uint center_index = SimulationDataOps::get_gauge_lattice_buffer_index(position);
        float4 gauge_state = float4(0, 0, 0, 0);
        GaugeSymmetriesVectorPack center_states = gauge_lattice_buffer[center_index];
        ...
        for (int z = -1; z <= 1; z++)
        {
            ...
            GaugeSymmetriesVectorPack neighboring_states = gauge_lattice_buffer[neighbor_index];
        ...
        gauge_lattice_buffer[center_index][symmetry_index] = gauge_state;
    }
