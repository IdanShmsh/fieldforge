...
{
    /// Implementation of fermion field rendering based on the norm of the fermion states.
    /// * Functions may read directly from and/or write directly to the simulation's lattice buffers and global values.
    namespace RenderFermionNorm
    {
        // Get the color at a given position based on the norm of the fermion state of the specified field.
        ...
        half4 get_fermion_norm_color_at_position(half3 position, uint field_index)
        {
            ...
            half state_norm = FermionFieldStateMath::norm(state);
            return state_norm * state_norm * fermion_field_properties[field_index].color;
        }
    }
}
